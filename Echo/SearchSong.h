...
public:
    ...
    static vector<Song*> searchByName(DoublyLinkedList<Song>& songs, const string& query) { //Busqueda rigida (Mas duro que el pan Dx)
        return search(songs, [&query](const Song& s) { return s.getName() == query; });
    }

	static vector<Song*> searchByNameContains(DoublyLinkedList<Song>& songs, const string& query) { // Busqueda por substring 
        return search(songs, [&query](const Song& s) {
            return s.getName().find(query) != string::npos;
            });
    }

    static vector<Song*> searchByNameIgnoreCase(DoublyLinkedList<Song>& songs, const string& query) { //Busqueda para niños uwu (ignora mayusculas y da con substring)
